...
def process_video(
    ...
    temp_output = "outputs/temp_video.mp4"
    ...
    # ---------------------------------------------------------
    # Convert to browser-compatible H264 (FIXED)
    # ---------------------------------------------------------

    ffmpeg_exe = imageio_ffmpeg.get_ffmpeg_exe()

    # استخدام أمر FFmpeg المباشر لتحويل الترميز بضبط yuv420p ليعمل على جميع المتصفحات
    cmd = [
        ffmpeg_exe,
        "-y",
        "-i",
    ...
    if os.path.exists(temp_output) and os.path.exists(output_path):
    ...
    if len(all_stats):
        ...
        avg_workers = np.mean([s["total_workers"] for s in all_stats])
        ...
        avg_compliance = (
            np.mean([s["compliance_rate"] for s in frames_with_people])
        ...
        summary = {
            ...
            "compliant_workers": sum(s["compliant_workers"] for s in all_stats),
            "violations": sum(s["violations"] for s in all_stats),
            "helmet_violations": sum(s["helmet_violations"] for s in all_stats),
            "vest_violations": sum(s["vest_violations"] for s in all_stats),
